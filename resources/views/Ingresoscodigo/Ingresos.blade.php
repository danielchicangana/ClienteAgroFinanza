...
<main class="container py-4">
    ...
    <div class="card p-3 mb-4">
        <div class="row g-2">
            ...
            <div class="col-md-2">
                <button class="btn btn-success w-100" onclick="agregarIngreso()">Añadir</button>
            </div>
        </div>
    </div>
    ...
    <!-- Filtro -->
    <div class="mb-3">
        <select id="filtro" class="form-select w-auto" onchange="filtrarIngresos()">
...
<script>
    ...
    function agregarIngreso() {
        ...
        ingresos.push({ descripcion, monto, fecha });
    ...
    function mostrarIngresos(lista) {
        ...
        lista.forEach((ingreso, index) => {
            total += ingreso.monto;

            let fila = `
                <tr>
                    <td>${ingreso.descripcion}</td>
                    <td>$${formatearNumero(ingreso.monto)}</td>
                    <td>${ingreso.fecha}</td>
                    <td>
                        <button class="btn btn-warning btn-sm" onclick="editarIngreso(${index})">Editar</button>
                        <button class="btn btn-danger btn-sm" onclick="eliminarIngreso(${index})">Eliminar</button>
                    </td>
                </tr>
            `;
    ...
    function guardarEdicion() {
        let nuevaDescripcion = document.getElementById("editarDescripcion").value.trim();
    ...
    function filtrarIngresos() {
        ...
        let filtrados = ingresos.filter(ingreso => {
            let fechaIngreso = ingreso.fecha;

            if (filtro === "diario") {
                const hoyStr = hoy.toISOString().slice(0, 10);
                return fechaIngreso === hoyStr;
            }
            if (filtro === "mensual") {
                let fechaObj = new Date(fechaIngreso);
